...
class Pago extends Conectar
{
  ...
  public function guardarDatosCobroPago($orden, $nota, $pago, $saldo)
  {
    //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
    //CUANDO ES APPWEB ES CONEXION.
    $data = NULL;
    ...
    parent::set_names();
    //QUERY
    $sql = "INSERT INTO tabla_cobranza_pago( orden, nota, pago, favor)  VALUES (?,?,?,?)";
    //PREPARACION DE LA CONSULTA PARA EJECUTARLA.
    $sql = $conectar->prepare($sql);
  ...
  public function anularPago($id, $estatus)
  {
    //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
    //CUANDO ES APPWEB ES CONEXION.
    $conectar = parent::conexion();
    parent::set_names();
    ///QUERY
    $sql = "UPDATE tabla_pago_data SET estatus=? WHERE id=?";
    //PREPARACION DE LA CONSULTA PARA EJECUTARLA.
    $sql = $conectar->prepare($sql);
    $sql->bindValue(1, $estatus);
    $sql->bindValue(2, $id);
  ...
  public function cargarDatosPago($id)
  {
    ...
    parent::set_names();
    //QUERY
    $sql = "SELECT A.id,A.fecha_registro,nota,A.fecha_pago,C.contrato,D.plan,D.costo, B.nombre_apel,B.documento,C.direccion,B.correo,B.telefono,E.forma,F.detalle,A.referencia,A.tasa,A.monto_dolar,A.monto_cambio,A.monto_pago,A.forma_pago,A.detalle_pago, A.contrato AS idcontrato, C.saldo 
            FROM tabla_pago_data AS A 
            INNER JOIN tabla_cliente_data AS B ON B.id=A.cliente
            INNER JOIN tabla_contrato_data AS C ON C.id=A.contrato
  ...
  public function cargarDatosPagoDetalle2($id)
  {
    ...
    parent::set_names();
    //QUERY
    $sql = "SELECT B.fecha_creacion, B.orden, B.detalle,C.fecha_registro ,A.pago, C.nota, A.favor
              FROM tabla_cobranza_pago AS A 
              INNER JOIN tabla_cobranza_data AS B ON B.id=A.orden
              INNER JOIN tabla_pago_data AS C ON C.id=A.nota
              WHERE A.nota = ? ";
  ...
  public function buscarCobros($id)
  {
    //LLAMAMOS A LA CONEXION QUE CORRESPONDA CUANDO ES SAINT: CONEXION2
    //CUANDO ES APPWEB ES CONEXION.
    $conectar = parent::conexion();
    parent::set_names();
    //QUERY
    $sql = "SELECT * FROM tabla_cobranza_pago WHERE nota=?";
    //PREPARACION DE LA CONSULTA PARA EJECUTARLA.
    $sql = $conectar->prepare($sql);
    $sql->bindValue(1, $id);
    $sql->execute();
  ...
  public function buscarContrato($id)
  {
    ...
    parent::set_names();
    //QUERY
    $sql = "SELECT contrato FROM tabla_cobranza_data WHERE id=?";
    //PREPARACION DE LA CONSULTA PARA EJECUTARLA.
    $sql = $conectar->prepare($sql);
    $sql->bindValue(1, $id);
    $sql->execute();
